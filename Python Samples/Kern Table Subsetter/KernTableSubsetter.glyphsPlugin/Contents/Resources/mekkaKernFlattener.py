...
def subsetKerning(kerningList, thisFont: GSFont):
	allowedPairs = set()
	for i, line in enumerate(worthKeepingText.splitlines()):
		...
		for i in range(0, len(line), 2):
			...
			allowedPairs.add((leftUnicode, rightUnicode))

	newKerning = list()
	for kernPair in kerningList:
		# print(kernPair)
		leftName = kernPair[0]
		rightName = kernPair[1]
		value = kernPair[2]
		...
		if leftGlyph and leftGlyph.unicode and rightGlyph and rightGlyph.unicode:
			if  (leftGlyph.unicode, rightGlyph.unicode) in allowedPairs:
				newKerning.append((leftName, rightName, value))
	print("Reduced the number of pairs:", len(kerningList), ">", len(newKerning))
	return newKerning