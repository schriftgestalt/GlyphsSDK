...
from GlyphsApp import Glyphs, GSInfoValue, GSFont, GSGlyph, GSLayer, GSFontMaster, GSGuide, GSAlignmentZone, GSAxis, GSClass, GSFeaturePrefix, GSFeature, GSInstance, GSMetric, GSGlyphInfo, GSAnnotation, GSHint, GSAnchor, GSMetricStore, GSPath, GSComponent, GSBackgroundImage, GSNode, GSGradient, GSControlLayer, GSUserNotification
from GlyphsApp import MOVE, LINE, CURVE, QCURVE, HOBBYCURVE, OFFCURVE, GSSHARP, GSSMOOTH, TAG, TOPGHOST, STEM, BOTTOMGHOST, FLEX, TTSNAP, TTANCHOR, TTSTEM, TTSHIFT, TTALIGN, TTINTERPOLATE, TTDIAGONAL, TTDELTA, CORNER, CAP, TTROUND, TTROUNDUP, TTROUNDDOWN, TTDONTROUND, TRIPLE, APP_MENU, FILE_MENU, EDIT_MENU, GLYPH_MENU, PATH_MENU, FILTER_MENU, VIEW_MENU, SCRIPT_MENU, WINDOW_MENU, HELP_MENU, DRAWFOREGROUND, DRAWBACKGROUND, DRAWINACTIVE, DOCUMENTOPENED, DOCUMENTACTIVATED, DOCUMENTWASSAVED, DOCUMENTEXPORTED, DOCUMENTCLOSED, TABDIDOPEN, TABWILLCLOSE, UPDATEINTERFACE, MOUSEMOVED, MOUSEDRAGGED, MOUSEDOWN, MOUSEUP, CONTEXTMENUCALLBACK, GSLowercase, OTF, TTF, WOFF, UFO, TEXT, MINUS, PLUS, CIRCLE, ARROW, divideCurve, distance, addPoints, scalePoint  # type: ignore
from GlyphsApp import DictProxy, ListProxy, OrderedDictProxy, FontGlyphsProxy, FontStemsProxy, FontAxesProxy, FontInstancesProxy, FontFontMasterProxy, FontClassesProxy, FontFeaturesProxy, FontFeaturePrefixesProxy, FontInfoPropertyProxy, UserDataProxy, PropertiesProxy
...
def test_GSFeature(font: GSFont):
	...
	assert isinstance(list(feature.labels), list)
...
def test_GSFontMaster(font: GSFont):
	...
	# iconName
	assert isinstance(master.iconName, str) or master.iconName is None
	# internalAxesValues / externalAxesValues
	assert len(master.internalAxesValues) == len(font.axes)
	for val in master.internalAxesValues:
		assert isinstance(val, float)
	assert len(master.externalAxesValues) == len(font.axes)
	for val in master.externalAxesValues:
		assert isinstance(val, float)
	# numbers
	assert isinstance(list(master.numbers), list)
	# metricValues
	assert master.metricValues is not None
	# guideLines (legacy alias of guides)
	assert len(master.guideLines) == len(master.guides)
...
def test_GSAlignmentZone(font: GSFont):
	...
	assert zone.plistValue() is not None
...
def test_GSInstance_related(font: GSFont):
	...
	assert instance.interpolatedFontProxy is not None
	assert_bool(instance, "exports")
	assert len(instance.internalAxesValues) == len(instance.font.axes)
	for val in instance.internalAxesValues:
		assert isinstance(val, float)
	assert len(instance.externalAxesValues) == len(instance.font.axes)
	for val in instance.externalAxesValues:
		assert isinstance(val, float)
...
def test_GSGlyph(font: GSFont):
	...
	assert_string(glyph, "topMetricsKey")
	assert_string(glyph, "bottomMetricsKey")
	assert_string(glyph, "group")
	assert_integer(glyph, "groupIdx")
	assert_bool(glyph, "storeGroup")
...
def test_layer_metrics_and_dimensions(layer: GSLayer):
	...
	assert layer.topMetricsKey is None or isinstance(layer.topMetricsKey, str)
	assert layer.bottomMetricsKey is None or isinstance(layer.bottomMetricsKey, str)

	assert isinstance(layer.bounds, NSRect)
	sel_bounds = layer.selectionBounds
	assert isinstance(sel_bounds, NSRect)
	assert sel_bounds.origin.x > 10000000
...
def test_layer_alignment_and_angle(layer: GSLayer):
	...
	assert isinstance(layer.isBraceLayer, bool)
	assert isinstance(layer.isBracketLayer, bool)

	assert isinstance(layer.italicAngle, float)
	assert_read_only(layer, "italicAngle")


def test_layer_axesValues(layer: GSLayer, font: GSFont):
	assert len(list(layer.axesValues)) == len(font.axes)
	for val in layer.axesValues:
		assert isinstance(val, float)


def test_layer_selection_and_changes(layer: GSLayer):
	...
	for anchor in layer.anchors:
		...
		selection += 1
	assert len(layer.selection) == selection
	layer.clearSelection()
	assert len(layer.selection) == 0
	layer.selection.append(layer.shapes[0])
	layer.selection.extend(list(layer.anchors))
	layer.selection.remove(layer.shapes[0])
	layer.selection.insert(0, layer.shapes[0])
	assert len(layer.selection) == 1 + len(layer.anchors)
	layer.clearSelection()

	# beginChanges/endChanges
	layer.beginChanges()
	layer.endChanges()
...
def test_layer_intersectionsBetweenPoints(layer: GSLayer):
	intersections = layer.intersectionsBetweenPoints((-1000, 105), (layer.width + 1000, 105))
	assert len(intersections) == 6
	assert intersections[1] == NSPoint(80, 105)
	assert intersections[-2] == NSPoint(369.0, 105)
...
def test_smartComponents(font: GSFont):
	...
	# GSGlyph.axes: glyph-local axes, works exactly like GSFont.axes
	glyph.axes = []
	assert len(glyph.axes) == 0
	assert_list(glyph.axes, test_values=[GSAxis(), GSAxis(), copy.copy(GSAxis())], assert_sorting=False)

	# Add axes

	axis1 = GSAxis()
	axis1.name = 'crotchDepth'
	glyph.axes.append(axis1)

	axis2 = GSAxis()
	axis2.name = 'shoulderWidth'
	glyph.axes.append(axis2)

	assert len(glyph.axes) == 2
	assert isinstance(axis1.axisId, str)

	crotch_depth_id = axis1.axisId
	shoulder_width_id = axis2.axisId

	# Position the smart layers in the design space (replaces the old pole mapping)

	for layer in glyph.layers:

		# NarrowShoulder layer
		if layer.name == 'NarrowShoulder':
			layer.attributes["coordinates"] = {crotch_depth_id: 0, shoulder_width_id: 0}

		# LowCrotch layer
		elif layer.name == 'LowCrotch':
			layer.attributes["coordinates"] = {crotch_depth_id: -100, shoulder_width_id: 100}

	# read back
	for layer in glyph.layers:
		if layer.name == 'NarrowShoulder':
			assert layer.attributes["coordinates"][crotch_depth_id] == 0
			assert layer.attributes["coordinates"][shoulder_width_id] == 0
		elif layer.name == 'LowCrotch':
			assert layer.attributes["coordinates"][crotch_depth_id] == -100
			assert layer.attributes["coordinates"][shoulder_width_id] == 100

	# the old pole mapping is gone
	with pytest.raises(NotImplementedError):
		_ = glyph.layers[0].smartComponentPoleMapping

	# GSComponent.smartComponentValues, keyed by axis.axisId
	layer = font.glyphs['n'].layers[0]
	component = layer.shapes[1]
	component.smartComponentValues[shoulder_width_id] = 30  # type: ignore
	component.smartComponentValues[crotch_depth_id] = -77  # type: ignore
	assert component.smartComponentValues[shoulder_width_id] == 30
	assert component.smartComponentValues[crotch_depth_id] == -77
...
def test_GSComponent_boolean_and_other_attributes(component_test_glyph_layer):
	...
	assert_bool(component, "automaticAlignment")

	# .alignment is a complex property; basic check
	assert component.alignment is not None

	assert_bool(component, "locked")
	assert_bool(component, "disableAlignment")
	assert component.shapeType is not None
	assert isinstance(component.componentMasterId, str) or component.componentMasterId is None
	_ = component.traverseAnchors  # just exercise the getter
...
def test_GSPath_attributes(path_test_layer):
	...
	assert_bool(path, "closed")

	assert path.direction in [1, -1]

	assert path.shapeType is not None
...
# GSNode Tests
@pytest.fixture
def node(path_test_layer):
	"""Provides the first node from the first path of the test layer."""
	path = cast(GSPath, path_test_layer.paths[0])
	return path.nodes[0]

def test_GSNode_attributes(node):
	assert repr(node) is not None
	assert isinstance(node.position, NSPoint)
	assert node.type in [LINE, CURVE, OFFCURVE, QCURVE]

	# Test mutability of smooth
	assert_bool(node, "smooth", node.smooth)

	# .selected is a UI property
	assert_bool(node, "selected", node.selected)

	# .name is an optional string property
	assert_string(node, "name", node.name)

def test_GSNode_relational_attributes(node):
	# index should be an integer and read-only
	assert isinstance(node.index, int)
	assert node.index is not None and node.index != NSNotFound # For a node in a path
	assert_read_only(node, 'index')

	assert isinstance(node.nextNode, GSNode)
	assert isinstance(node.prevNode, GSNode)

	# GSNode.glyph / GSNode.connection (deprecated alias for smooth)
	assert node.glyph is not None
...
def test_GSNode_userData(node):
	assert node.userData is not None
	node.userData["TestData"] = 42
	assert node.userData["TestData"] == 42
	del node.userData["TestData"]
	assert node.userData.get("TestData") is None

def test_GSNode_methods(node):
	# These methods modify the path structure, use with care or on a copy
	path = node.parent
	path_copy = copy.deepcopy(path)
	test_index = node.index
	...
	original_type = node_in_copy.type
...
def test_GSGuide_attributes(path_test_layer):
	...
	assert_bool(guide, "showMeasurement")
	# GSElement.orientation
	assert isinstance(guide.orientation, int)
...
def test_GSLayer_backgroundImage(font: GSFont):
	...
	image.scaleHeightToEmUnits(layer.ascender - layer.descender)
...
def test_GSGlyphInfo_attributes(font: GSFont):
	...
	if info_lam_alef:  # This might not be in the default glyph data
		...
		assert info_lam_alef.sortName == "ar0900_ar0009"
		assert info_lam_alef.sortNameKeep == "ar0900_ar0009"
		assert info_lam_alef.altNames[0] == "lamalefisolatedarabic"

def test_GSFont_more_properties(font: GSFont):
	# characterForGlyph
	assert font.characterForGlyph(font.glyphs["a"]) == ord("a")
	# kerningLTR (same storage as font.kerning)
	assert_dict(font.kerningLTR)
	# gridSubDivision (alias of gridSubDivisions)
	assert font.gridSubDivision == font.gridSubDivisions
	# displayStrings
	assert isinstance(font.displayStrings, list)
	# numbers
	assert isinstance(list(font.numbers), list)
	# disablesAutomaticAlignment
	assert_bool(font, "disablesAutomaticAlignment")


def test_GSHint(font: GSFont):
	layer = font.glyphs['a'].layers[0]
	layer.hints = []
	hint = GSHint()
	hint.originNode = layer.shapes[0].nodes[0]
	hint.targetNode = layer.shapes[0].nodes[1]
	hint.type = STEM
	layer.hints.append(hint)
	hint = layer.hints[0]
	assert hint.__repr__() is not None

	# parent
	assert hint.parent == layer

	# type
	assert isinstance(hint.type, int)

	# horizontal
	assert_bool(hint, "horizontal")

	# name
	assert hint.name is None or isinstance(hint.name, str)
	hint.name = 'testHint'
	assert hint.name == 'testHint'

	# options / alignment / stem
	assert isinstance(hint.options, int)
	assert isinstance(hint.alignment, int)
	assert isinstance(hint.stem, int)

	# isTrueType / isPostScript / isCorner (read-only)
	assert isinstance(hint.isTrueType, bool)
	assert isinstance(hint.isPostScript, bool)
	assert isinstance(hint.isCorner, bool)

	# nodes
	assert isinstance(hint.originNode, GSNode)
	assert isinstance(hint.targetNode, GSNode)
	# unused for a stem hint
	assert hint.otherNode1 is None or isinstance(hint.otherNode1, GSNode)
	assert hint.otherNode2 is None or isinstance(hint.otherNode2, GSNode)

	# indexes
	assert hint.originIndex is not None
	assert hint.targetIndex is not None
	_ = hint.otherIndex1
	_ = hint.otherIndex2
	assert isinstance(hint.origin, (list, type(None)))
	assert isinstance(hint.target, (list, type(None)))
	assert isinstance(hint.other1, (list, type(None)))
	assert isinstance(hint.other2, (list, type(None)))

	# scale / position / width (just exercise the getters)
	assert hint.scale is not None
	_ = hint.position
	_ = hint.width

	# tempData
	hint.tempData['testKey'] = 42
	assert hint.tempData['testKey'] == 42
	del hint.tempData['testKey']
...
def test_GSGradient():
	gradient = GSGradient()
	gradient = copy.copy(gradient)
	assert gradient.__repr__() is not None

	# type
	assert isinstance(gradient.type, int)
	gradient.type = 1
	assert gradient.type == 1
	gradient.type = 0

	# start / end
	gradient.start = NSPoint(0, 0)
	assert gradient.start == NSPoint(0, 0)
	gradient.end = NSPoint(1, 1)
	assert gradient.end == NSPoint(1, 1)

	# absoluteStart / absoluteEnd (just exercise the getters)
	_ = gradient.absoluteStart
	_ = gradient.absoluteEnd

	# colors
	assert isinstance(list(gradient.colors), list)


def test_GSPathPen(font: GSFont):
	new_glyph = GSGlyph('penTest')
	font.glyphs.append(new_glyph)
	layer = new_glyph.layers[0]
	layer.shapes = []

	# GSLayer.getPointPen() + point pen protocol (beginPath/addPoint/endPath)
	pen = layer.getPointPen()
	pen.beginPath()
	...
	assert len(layer.paths[0].nodes) == 3

	# GSLayer.getPen() + segment pen protocol (moveTo/lineTo/curveTo/closePath)
	pen = layer.getPen()
	pen.moveTo((0, 0))
	pen.lineTo((100, 0))
	pen.curveTo((110, 30), (110, 70), (100, 100))
	pen.closePath()
	assert len(layer.paths) == 2

	del font.glyphs['penTest']


def test_GSMetricStore():
	metric_store = GSMetricStore(750, 12)
	assert metric_store.__repr__() is not None

	# position
	assert metric_store.position == 750
	assert_float(metric_store, "position")

	# overshoot
	assert metric_store.overshoot == 12
	assert_float(metric_store, "overshoot")

	# size (compatibility alias of overshoot)
	assert metric_store.size == metric_store.overshoot

	# name (read-only, derived from title())
	assert metric_store.name is None or isinstance(metric_store.name, str)

	# filter (NSPredicate or None; None for a standalone store)
	assert metric_store.filter is None

	# metric (GSMetric or None; None for a standalone store)
	assert metric_store.metric is None or isinstance(metric_store.metric, GSMetric)


def test_GSControlLayer():
	newline_layer = GSControlLayer.newline()
	assert newline_layer is not None
	placeholder_layer = GSControlLayer.placeholder()
	assert placeholder_layer is not None
	_ = newline_layer.bezierPath  # None for control layers


def test_GSUserNotification():
	# construct and populate a notification without delivering it
	notification = GSUserNotification.new()
	notification.title = 'Glyphs Unit Test'
	assert notification.title == 'Glyphs Unit Test'
	notification.message = 'Hello World'
	assert notification.message == 'Hello World'
	notification.actionButtonTitle = 'OK'
	assert notification.actionButtonTitle == 'OK'
	notification.userInfo = {'testKey': 'testValue'}
	assert notification.userInfo['testKey'] == 'testValue'


def test_GSInfoValue(font: GSFont):
	font.familyNames["ENG"] = "English Family"
	info_value = font.familyNames["ENG"]
	assert isinstance(info_value, GSInfoValue)
	assert info_value.value == "English Family"
	assert info_value.languageTag == "ENG"
	assert info_value.key is None or isinstance(info_value.key, str)

	# GSInfoValueLocalized
	for prop in font.properties:
		if hasattr(prop, "values") and len(prop.values):
			assert prop.key is None or isinstance(prop.key, str)
			_ = prop.defaultValue
			break

	del font.familyNames["ENG"]
...
def test_Constants_are_defined():
	from GlyphsApp import (
		GSMOVE, GSLINE, GSCURVE, GSQCURVE, GSOFFCURVE, GSHOBBYCURVE,
	...
	constants = [
		GSMOVE, GSLINE, GSCURVE, GSQCURVE, GSOFFCURVE, GSHOBBYCURVE, GSSHARP, GSSMOOTH,
